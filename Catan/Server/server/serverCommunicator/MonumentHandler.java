...
public class MonumentHandler implements HttpHandler {
	...
	public MonumentHandler(ITranslator translator, IMovesFacade movesFacade, IMovesCommandLog movesLog) {
	...
	public void handle(HttpExchange exchange) throws IOException {
		System.out.println("In monument handler");
		...
		if(exchange.getRequestMethod().toLowerCase().equals("post")) {
			...
			try { 
				...
				MonumentDevRequest request = (MonumentDevRequest) translator.translateFrom(requestJson.toString(), MonumentDevRequest.class);
				exchange.getRequestBody().close();
				
				ServerModel serverModel = this.movesFacade.monument(request, cookie);
				...
				System.out.println("unrecognized / invalid monument request");
