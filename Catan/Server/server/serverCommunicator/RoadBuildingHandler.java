...
/**
 * Handler for RoadBuilding command
 * @author Chad
 *
 */
public class RoadBuildingHandler implements HttpHandler {
	...
	public RoadBuildingHandler(ITranslator translator, IMovesFacade movesFacade, IMovesCommandLog movesLog) {
	...
	public void handle(HttpExchange exchange) throws IOException {
		...
		if(exchange.getRequestMethod().toLowerCase().equals("post")) {
			try { 
				...
				RoadBuildingDevRequest request = (RoadBuildingDevRequest) translator.translateFrom(requestJson.toString(), RoadBuildingDevRequest.class);
				exchange.getRequestBody().close();
				
				ServerModel serverModel = this.movesFacade.roadBuilding(request, cookie);
				...
				System.out.println("unrecognized / invalid road building(dev) request");
